...
export default function Edit({ attributes, setAttributes }) {
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				let response = await fetch("/wp-json/easyiconfonts/v1/loaded-fonts");

				if (!response.ok) {
					response = await fetch("/?rest_route=/easyiconfonts/v1/loaded-fonts");
				}
	...
	const isIconValid = (iconClassName, loadedFonts) => {
		for (const fontFolder in loadedFonts) {
			...
			for (const glyphName in fontGlyphs) {
				const expectedClass = `eif-${fontFolder.toLowerCase()}__${glyphName}`;
								...
								<details key={index} className="eif-font-details">
									...
									<div className="eif-font-icons">
										{font.glyphs.map(([name], i) => {
											const iconClass = `eif-${font.fontFolder.toLowerCase()}__${name}`;
